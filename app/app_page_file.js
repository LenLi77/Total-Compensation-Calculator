...
export default function TotalCompensationCalculator() {
    ...
    switch(country) {
      case 'EE':
        return {
          ...
          carTaxable: true,
          carTaxRate: 0.705, // 22/78 CIT + 33% social = ~70.5%
          mealTaxable: true,
          fringeBenefitTax: 0.705 // Same as car
        };
      case 'LV':
        return {
          ...
          carTaxable: true,
          carTaxRate: 0.50, // Estimated - treated as non-business expense
          mealTaxable: true,
      ...
      case 'LT':
        return {
          ...
          healthTaxFreeMonthly: 0,
          carTaxable: true,
          carTaxRate: 0.40, // Estimated - subject to income tax + social
          mealTaxable: true,
  ...
  // Car - Different calculation based on country
  let carBIK = 0;
  let carUsage = 0;
  let carTaxCost = 0;
  ...
  if (benefits.car) {
    ...
    if (country === 'EE') {
      carBIK = carValue * 0.012; // 1.2% monthly benefit-in-kind
      carUsage = carValue * 0.03; // 3% monthly usage value (not taxed)
      carTaxCost = carBIK * rules.carTaxRate; // Employer pays tax on BIK
    } else {
      // For LV/LT, simplified: full value is taxable benefit
      carBIK = carValue * 0.015; // Estimated monthly benefit
      carTaxCost = carBIK * rules.carTaxRate;
    }
  }

  // Meal vouchers - Different treatment by country
  const mealValue = benefits.meal ? (parseFloat(benefits.mealAmount) || 0) * workDays : 0;
  let mealTaxCost = 0;
  if (benefits.meal && rules.mealTaxable) {
    mealTaxCost = mealValue * (country === 'EE' ? rules.fringeBenefitTax : 0.20); // Simplified for LV/LT
  }

  // Health - Country-specific tax treatment
  ...
  // Totals - Employee receives gross value
  ...
  // Employer costs (taxes on fringe benefits)
